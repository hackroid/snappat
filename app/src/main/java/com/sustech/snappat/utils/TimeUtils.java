 ...
 *
 * 将时间戳转为时间字符串 milliseconds2String 将时间字符串转为时间戳 string2Milliseconds 将时间字符串转为Date类型 string2Date
 * 将Date类型转为时间字符串 date2String 将Date类型转为时间戳 date2Milliseconds 将时间戳转为Date类型 milliseconds2Date
 * 毫秒时间戳单位转换（单位：unit） milliseconds2Unit 获取两个时间差（单位：unit） getIntervalTime 获取当前时间
 * getCurTimeMills、getCurTimeString、getCurTimeDate 获取与当前时间的差（单位：unit） getIntervalByNow 判断闰年
 * isLeapYear
 */
public class TimeUtils {
   ...
   * </p>
   * 定义了以下模式字母（所有其他字符 'A' 到 'Z' 和 'a' 到 'z' 都被保留）： <br> <table border="1" cellspacing="1"
   * cellpadding="1" summary="Chart shows pattern letters, date/time component, presentation, and
   * examples.">
   * <tr>
   * <th align="left">字母</th>
